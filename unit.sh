...
echo "### Running pylint"
pylint -E -e F -f parseable $MOZHARNESS_PY_FILES $SCRIPTS_PY_FILES 2>&1 | egrep -v "(No config file found, using default configuration|Instance of .SplitResult. has no .path. member|Unable to import 'devicemanager|Undefined variable 'devicemanager')"

rm -rf build logs localconfig.py
if [ $OS_TYPE != 'windows' ] ; then
